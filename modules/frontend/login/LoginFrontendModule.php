...
class LoginFrontendModule extends DynamicFrontendModule {
  ...
  public function renderFrontend($sAction = 'login') {
    ...
    if(Session::getSession()->getUser() && Session::getSession()->getUser()->mayViewPage($this->oPage)) {
      ...
      $oTemplate->replaceIdentifier('action', Util::link($this->oPage->getFullPathArray(), null, array('logout' => 'true')));
      return $oTemplate;
    }
    ...
    $oTemplate->replaceIdentifier('action', Util::linkToSelf(null, null, true));
    if($sAction === 'login') {

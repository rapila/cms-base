...
class DefaultPageTypeModule extends PageTypeModule {
  ...
  private function fillContainerWithModule($oContentObject, $oTemplate, $iModuleId) {
    ...
    if(Session::getSession()->isAuthenticated() && Session::getSession()->getUser()->mayEditPageContents($oContentObject->getPage())) {
      ...
      $oTag = TagWriter::quickTag("a", array("href" => Util::link(array('content', $oContentObject->getPage()->getId(), 'edit', $oContentObject->getId()), "BackendManager"), 'style' => 'z-index:1000;padding:0;margin:-6px 0 0 0;display:block;text-decoration:none;border:none;line-height:0;font-size:1px;clear:both;', "title" => StringPeer::getString("content_edit")), $sEditImage);

	...
	public function getDetail() {
		if(!$this->oPage) {
			...
			if(isset($_REQUEST['error404'])) {
				...
				$oTemplate->replaceIdentifier('create_link', TagWriter::quickTag('a', array('class' => 'edit_related_link highlight', 'href' => LinkUtil::link('pages/new', null, array('action' => 'create', 'parent_id' => PagePeer::getRootPage()->getId()))), StringPeer::getString('page.create_child', null,null,array('title' => PagePeer::getRootPage()->getPageTitle()))));
		...
		$oTemplate->replaceIdentifier('module_info_link', TagWriter::quickTag('a', array('title' => StringPeer::getString('module_info'), 'class' => 'info', 'href' => LinkUtil::link('pages', null, array('get_module_info' => 'true')))));
		...
		} else if(isset($_REQUEST['action']) && isset($_REQUEST['parent_id'])) {
			$oParent = PagePeer::retrieveByPK($_REQUEST['parent_id']);
			if($oParent) {
				$sWeblink = BackendManager::getWebLink($oParent->getFullPathArray());
				$oTemplate->replaceIdentifier("display_weblink_info", $_SERVER['HTTP_HOST'].$sWeblink.'/<span>???</span>', null, Template::NO_HTML_ESCAPE);
			}
			$oTemplate->replaceIdentifier("name", $sName);
		}
		...
		if($this->oPage->getId() !== null){
			...
			/* @todo bug #0000108
			 * check quickfix: if a user has no admin right, but full right in a group "administrator" and you click on the rootpage, 
			 * there is no parent and this throws an fatal error
			 * is this the right place to fix?
			 */
			$oParent = $this->oPage->getParent() !== null ? $this->oPage->getParent() : $this->oPage;
			$oTemplate->replaceIdentifier("sort_option_disabled", (Session::getSession()->getUser()->mayEditPageDetails($oParent) ? '' : ' disabled="disabled"'), null, Template::NO_HTML_ESCAPE);

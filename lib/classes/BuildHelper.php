...
EOT;
		...
		$aSchemas = array();
		$aSchemaFiles = ResourceFinder::create(array(DIRNAME_CONFIG))->addExpression('/(.+\\.)?schema\\.xml/')->noCache()->all()->returnObjects()->find();
		foreach($aSchemaFiles as $oSchemaFile) {
			$sSchemaName = $oSchemaFile->getFileName('.xml');
			if($sSchemaName === 'schema') {
				$sSchemaName = 'rapila';
			} else {
				$sSchemaName = substr($sSchemaName, 0, -strlen('.schema'));
			}
			if(!isset($aSchemas[$sSchemaName])) {
				$aSchemas[$sSchemaName] = new Template($sSchemaTemplate, null, true);
				$aSchemas[$sSchemaName]->replaceIdentifier('comment', "This file is generated by the generate-model.sh script, edit schema.xml in the config dir or the plugins or site's schema.xml file instead", null, Template::NO_HTML_ESCAPE);
				$aSchemas[$sSchemaName]->replaceIdentifier('name', $sSchemaName);
			}
			$aSchemas[$sSchemaName]->replaceIdentifierMultiple('schema_content', file_get_contents($oSchemaFile->getFullPath()), null, Template::NO_HTML_ESCAPE);
		}
		foreach($aSchemas as $sSchemaName => $oSchemaTemplate) {
	...
	*/
	private static function deleteUnusedFiles($bIsDevVersion = false) {
		...
		$aSchemas = ResourceFinder::findResourceObjectsByExpressions(array(DIRNAME_GENERATED, '/.+\\.schema.xml/'), ResourceFinder::SEARCH_MAIN_ONLY);

...
class TagPeer extends BaseTagPeer {
	...
	*/
	public static function allTaggedIdsForModel($sModelName, $sTagName = null) {
		$oCriteria = $sInstanceQuery::create()->innerJoinTag()->filterByModelName($sModelName);
		if($sTagName !== null) {
	...
	public static function droppedOnto($mDroppedId, $sDroppableModelName, $sTagName) {
		$sInstanceModel = $sDroppableModelName::TAG_INSTANCE_MODEL_NAME;
		$sInstanceQuery = "${sInstanceModel}Query";

		$oQuery = $sInstanceQuery::create()->useTagQuery()->filterByName($mDroppedId)->endUse();
		...
		$oResult->is_first_of_model = $sInstanceQuery::create()->filterByModelName($sDroppableModelName)->count() === 0;
		$oInstance = $sDroppableModelName::addTagTo($sTagName, $mDroppedId);
		return $oResult;
	}

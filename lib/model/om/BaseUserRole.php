...
abstract class BaseUserRole extends BaseObject implements Persistent
{
    ...
    public function save(PropelPDO $con = null)
    {
        ...
        try {
            ...
            if ($isInsert) {
                ...
                // denyable behavior
                if(!(UserRolePeer::isIgnoringRights() || $this->mayOperate("insert"))) {
                ...
                // denyable behavior
                if(!(UserRolePeer::isIgnoringRights() || $this->mayOperate("update"))) {
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(UserRolePeer::ROLE_KEY)) {
        ...
        $sql = sprintf(
            'INSERT INTO `user_roles` (%s) VALUES (%s)',
    ...
    public function mayOperate($sOperation, $oUser = false) {
        $oUser = UserRolePeer::getRightsUser($oUser);
        $bIsAllowed = false;
        if($oUser && ($this->isNew() || $this->getCreatedBy() === $oUser->getId()) && UserRolePeer::mayOperateOnOwn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        } else if(UserRolePeer::mayOperateOn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        }
        FilterModule::getFilters()->handleUserRoleOperationCheck($sOperation, $this, $oUser, array(&$bIsAllowed));
        return $bIsAllowed;
    }

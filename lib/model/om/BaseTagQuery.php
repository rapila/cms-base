...
abstract class BaseTagQuery extends ModelCriteria
{
	...
	{
		$sql = 'SELECT `ID`, `NAME`, `CREATED_AT`, `UPDATED_AT`, `CREATED_BY`, `UPDATED_BY` FROM `tags` WHERE `ID` = :p0';
		try {
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new Tag();
			$obj->hydrate($row);
			TagPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
	...
	public function filterByUserRelatedByCreatedBy($user, $comparison = null)
	{
		if ($user instanceof User) {
			...
			return $this
				->addUsingAlias(TagPeer::CREATED_BY, $user->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
	...
	public function filterByUserRelatedByUpdatedBy($user, $comparison = null)
	{
		if ($user instanceof User) {
			...
			return $this
				->addUsingAlias(TagPeer::UPDATED_BY, $user->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
	...
	public function filterByTagInstance($tagInstance, $comparison = null)
	{
		...
		} elseif ($tagInstance instanceof PropelCollection) {
			return $this
				->useTagInstanceQuery()
				->filterByPrimaryKeys($tagInstance->getPrimaryKeys())
				->endUse();
		} else {
			throw new PropelException('filterByTagInstance() only accepts arguments of type TagInstance or PropelCollection');
		}
	}

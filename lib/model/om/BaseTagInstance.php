...
include_once 'model/TagInstancePeer.php';
...
abstract class BaseTagInstance extends BaseObject  implements Persistent {
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->tagged_item_id = $rs->getInt($startcol + 1);

			$this->model_name = $rs->getString($startcol + 2);
			...
			return $startcol + 4; // 4 = TagInstancePeer::NUM_COLUMNS - TagInstancePeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			TagInstancePeer::doDelete($this, $con);
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = TagInstancePeer::doInsert($this, $con);
					...
					$affectedRows += TagInstancePeer::doUpdate($this, $con);
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setTagId(NULL); // this is a pkey column, so set to default value

		$copyObj->setTaggedItemId(NULL); // this is a pkey column, so set to default value

		$copyObj->setModelName(NULL); // this is a pkey column, so set to default value
	...
	public function getTag($con = null)
	{
		...
		include_once 'model/om/BaseTagPeer.php';

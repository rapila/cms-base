...
include_once 'model/TagPeer.php';
...
abstract class BaseTag extends BaseObject  implements Persistent {
		...
		if ($this->collTagInstances === null) {
			if ($this->isNew()) {
			   $this->collTagInstances = array();
				...
				$criteria->add(TagInstancePeer::TAG_ID, $this->getId());
	 ...
	 */
	public function getTagInstancesJoinUser($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'model/om/BaseTagInstancePeer.php';
		if ($criteria === null) {
		...
		if ($this->collTagInstances === null) {
			if ($this->isNew()) {
				...
				$this->collTagInstances = TagInstancePeer::doSelectJoinUser($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last

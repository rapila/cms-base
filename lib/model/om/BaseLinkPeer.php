...
// actual class may be a subclass -- as returned by LinkPeer::getOMClass()
...
abstract class BaseLinkPeer {
	...
	{
		include_once 'model/map/LinkMapBuilder.php';
		return BasePeer::getMapBuilder('model.map.LinkMapBuilder');
	}
	/**
	...
	const COUNT = 'COUNT(links.ID)';
	const COUNT_DISTINCT = 'COUNT(DISTINCT links.ID)';
	...
	public static function doCountJoinUserRelatedByUpdatedBy(Criteria $criteria, $distinct = false, $con = null)
	{
		...
		$criteria->addJoin(LinkPeer::UPDATED_BY, UserPeer::ID);
		...
		while($rs->next()) {
			...
			foreach($results as $temp_obj1) {
				...
				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					$newObject = false;
					// e.g. $author->addBookRelatedByBookId()
					$temp_obj2->addLinkRelatedByOwnerId($obj1); //CHECKME
					break;
				}
			}
			if ($newObject) {
				$obj2->initLinksRelatedByOwnerId();
				$obj2->addLinkRelatedByOwnerId($obj1); //CHECKME
			}
			$results[] = $obj1;
		}
		return $results;
		...
		while($rs->next()) {
			...
			foreach($results as $temp_obj1) {
				...
				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					$newObject = false;
					// e.g. $author->addBookRelatedByBookId()
					$temp_obj2->addLinkRelatedByCreatedBy($obj1); //CHECKME
					break;
				}
			}
			if ($newObject) {
				$obj2->initLinksRelatedByCreatedBy();
				$obj2->addLinkRelatedByCreatedBy($obj1); //CHECKME
			}
			$results[] = $obj1;
		}
		return $results;
	...
	public static function doSelectJoinUserRelatedByUpdatedBy(Criteria $c, $con = null)
	{
		...
		$c->addJoin(LinkPeer::UPDATED_BY, UserPeer::ID);
		$rs = BasePeer::doSelect($c, $con);
		$results = array();
		...
		while($rs->next()) {
			...
			foreach($results as $temp_obj1) {
				...
				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					$newObject = false;
					// e.g. $author->addBookRelatedByBookId()
					$temp_obj2->addLinkRelatedByUpdatedBy($obj1); //CHECKME
					break;
				}
			}
			if ($newObject) {
				$obj2->initLinksRelatedByUpdatedBy();
				$obj2->addLinkRelatedByUpdatedBy($obj1); //CHECKME
			}
			$results[] = $obj1;
		}
		return $results;
		...
		while($rs->next()) {
			...
			for ($j=0, $resCount=count($results); $j < $resCount; $j++) {
				...
				if ($temp_obj3->getPrimaryKey() === $obj3->getPrimaryKey()) {
					$newObject = false;
					$temp_obj3->addLinkRelatedByOwnerId($obj1); // CHECKME
					break;
				}
			}

			if ($newObject) {
				$obj3->initLinksRelatedByOwnerId();
			...
			for ($j=0, $resCount=count($results); $j < $resCount; $j++) {
				...
				if ($temp_obj5->getPrimaryKey() === $obj5->getPrimaryKey()) {
					$newObject = false;
					$temp_obj5->addLinkRelatedByCreatedBy($obj1); // CHECKME
					break;
				}
			}

			if ($newObject) {
				$obj5->initLinksRelatedByCreatedBy();
			...
			for ($j=0, $resCount=count($results); $j < $resCount; $j++) {
				...
				if ($temp_obj6->getPrimaryKey() === $obj6->getPrimaryKey()) {
					$newObject = false;
					$temp_obj6->addLinkRelatedByUpdatedBy($obj1); // CHECKME
					break;
				}
			}

			if ($newObject) {
				$obj6->initLinksRelatedByUpdatedBy();
	...
	public static function doSelectJoinAllExceptLanguage(Criteria $c, $con = null)
	{
		...
		while($rs->next()) {
			...
			for ($j=0, $resCount=count($results); $j < $resCount; $j++) {
				...
				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					$newObject = false;
					$temp_obj2->addLinkRelatedByOwnerId($obj1);
					break;
				}
			}
			...
			for ($j=0, $resCount=count($results); $j < $resCount; $j++) {
				...
				if ($temp_obj4->getPrimaryKey() === $obj4->getPrimaryKey()) {
					$newObject = false;
					$temp_obj4->addLinkRelatedByCreatedBy($obj1);
					break;
				}
			}

			if ($newObject) {
				$obj4->initLinksRelatedByCreatedBy();
			...
			for ($j=0, $resCount=count($results); $j < $resCount; $j++) {
				...
				if ($temp_obj5->getPrimaryKey() === $obj5->getPrimaryKey()) {
					$newObject = false;
					$temp_obj5->addLinkRelatedByUpdatedBy($obj1);
					break;
				}
			}

			if ($newObject) {
				$obj5->initLinksRelatedByUpdatedBy();
	...
	public static function doSelectJoinAllExceptDocumentCategory(Criteria $c, $con = null)
	{
		...
		while($rs->next()) {
			...
			for ($j=0, $resCount=count($results); $j < $resCount; $j++) {
				...
				if ($temp_obj3->getPrimaryKey() === $obj3->getPrimaryKey()) {
					$newObject = false;
					$temp_obj3->addLinkRelatedByOwnerId($obj1);
					break;
				}
			}
...
if (Propel::isInit()) {
	...
	require_once 'model/map/LinkMapBuilder.php';
	Propel::registerMapBuilder('model.map.LinkMapBuilder');
}

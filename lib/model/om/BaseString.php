...
include_once 'model/StringPeer.php';
...
abstract class BaseString extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the string_key field.
	 * @var        string
	 */
	...
	/**
	 * Get the [string_key] column value.
	 * 
	 * @return     string
	 */
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->language_id = $rs->getString($startcol + 0);

			$this->string_key = $rs->getString($startcol + 1);
			...
			return $startcol + 3; // 3 = StringPeer::NUM_COLUMNS - StringPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = StringPeer::doInsert($this, $con);
					...
					$affectedRows += StringPeer::doUpdate($this, $con);
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setStringKey(NULL); // this is a pkey column, so set to default value

...
abstract class BaseGroupRole extends BaseObject implements Persistent
{
    ...
    public function save(PropelPDO $con = null)
    {
        ...
        try {
            ...
            if ($isInsert) {
                ...
                // denyable behavior
                if(!(GroupRolePeer::isIgnoringRights() || $this->mayOperate("insert"))) {
                ...
                // denyable behavior
                if(!(GroupRolePeer::isIgnoringRights() || $this->mayOperate("update"))) {
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(GroupRolePeer::GROUP_ID)) {
            $modifiedColumns[':p' . $index++]  = '`group_id`';
        }
        if ($this->isColumnModified(GroupRolePeer::ROLE_KEY)) {
        ...
        $sql = sprintf(
            'INSERT INTO `group_roles` (%s) VALUES (%s)',
    ...
    public function mayOperate($sOperation, $oUser = false) {
        $oUser = GroupRolePeer::getRightsUser($oUser);
        $bIsAllowed = false;
        if($oUser && ($this->isNew() || $this->getCreatedBy() === $oUser->getId()) && GroupRolePeer::mayOperateOnOwn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        } else if(GroupRolePeer::mayOperateOn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        }
        FilterModule::getFilters()->handleGroupRoleOperationCheck($sOperation, $this, $oUser, array(&$bIsAllowed));
        return $bIsAllowed;
    }

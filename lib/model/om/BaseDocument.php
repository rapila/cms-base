...
abstract class BaseDocument extends BaseObject implements Persistent
{
     ...
     */
    public function getContentCreatedAt($format = 'H:i:s')
    {
    ...
    /**
     * @return All tags for the current Document
     */
     ...
     */
    public static function addTagTo($sDocumentId, $mTag)
    {
        ...
        $oTagInstance = TagInstanceQuery::create()->findPk(array($oTag->getId(), $sDocumentId, "Document"));
        if($oTagInstance !== null) {
            return $oTagInstance;
        }
        $oTagInstance = new TagInstance();
        $oTagInstance->setTag($oTag);
        $oTagInstance->setModelName("Document");
        $oTagInstance->setTaggedItemId($sDocumentId);
        $oTagInstance->save();
        return $oTagInstance;
    }
    /**
     ...
     */
    public static function removeTagFrom($sDocumentId, $mTag)
    {
        if(is_string($mTag)) {
     ...
     */
    public static function removeAllTagsFrom($sDocumentId)
    {
        $aTagInstances = self::tagsFor($sDocumentId, 'instances');
    ...
    public function mayOperate($sOperation, $oUser = false) {
        ...
        if($oUser && ($this->isNew() || $this->getCreatedBy() === $oUser->getId()) && DocumentPeer::mayOperateOnOwn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        } else if(DocumentPeer::mayOperateOn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        }
